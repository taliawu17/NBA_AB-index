...
3. **`ExtractKeyInfo_playerN.py`** ÔåÆ slim key logs ÔåÆ add `age_at_game`, `age_years`, `age_days` in **Excel** ÔåÆ `playerN_key_game_logs_age.csv`.
4. Select **top-2 games per season + witness games** ÔåÆ `top_games_per_season_playerN.csv`.
5. Calculate AB-index ÔåÆ `playerN_index.csv` (includes `personId`).
6. Rank valid indices (`index >= 18`) ÔåÆ `playerN_index_ranked.csv`.
7. **Combine** all batches, dedupe by `personId`, rank ÔåÆ `player_index_all.csv` / `player_index_ranked_all.csv`.

Batch 3 overlaps **batch 1** (same `personId`s); step 7 keeps one row per player (highest `index`, then `age_years`, then `age_days`).

Index rule: \(k = \max \min(\text{age\_years}, \text{points})\) over top games; \(k < 18\) ÔåÆ `NA`. Code: `db_scripts/calculate_indices.py`.

---

## Scripts (steps 1ÔÇô3 by batch)
...
**Key logs:** `playerN_key_game_logs.csv` ÔåÆ Excel ÔåÆ `playerN_key_game_logs_age.csv`

---

## Steps 4ÔÇô7 (same pattern; change `N` = 1, 2, 3)

**Step 4 ÔÇö `db_scripts/select_top_games_per_season_playerN.py`** (`top_games_selection.py`):  
Per player-season: **top 2** games by `points` (tie-break: later `gameDate`) Ôê¬ **witness** games (`points >= age_years`), then dedupe.

**Step 5 ÔÇö `db_scripts/calculate_indices_playerN.py`:**  
Input: `top_games_per_season_playerN.csv` ÔåÆ output `playerN_index.csv`.  
...
**Step 7 ÔÇö combine** (reads `player1_index.csv`, `player2_index.csv`, `player3_index.csv`, not the per-batch ranked files):
...
python db_scripts/build_3rankings.py            # -> 3rankings_20260518.csv (NBA75 Ôê¬ B100 Ôê¬ ABÔëÑ33)
...
| `player_index_ranked_all.csv` | Combined valid indices + `AB_rank` 1ÔÇªN |
