#!/usr/bin/env python3
"""
Batch 3: top 2 + witness games (scheme 1).

Input:  output/player3_key_game_logs_age.csv  (age_years from Excel)
Output: output/top_games_per_season_player3.csv
"""
...
KEY_LOGS_FILE = OUTPUT_DIR / "player3_key_game_logs_age.csv"
OUTPUT_FILE = OUTPUT_DIR / "top_games_per_season_player3.csv"
...
def main() -> None:
    ...
    cols = set(df.columns)
    has_game_date = "gameDate" in cols or "gameDateTimeEst" in cols
    if not has_game_date or not {"year", "points", "age_years"} <= cols:
        raise ValueError(
            f"Missing required columns: {sorted({'year', 'points', 'age_years', 'gameDate'} - cols)}"
        )
